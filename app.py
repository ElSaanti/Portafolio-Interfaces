...
def screenshot_url(site_url):
    encoded = site_url.replace("https://", "").replace("http://", "").replace("/", "%2F").replace(":", "%3A")
    return f"https://api.apiflash.com/v1/urltoimage?access_key=free&url={site_url}&width=800&height=500&format=jpeg&fresh=false"

def render_card(project, index):
    num_str = str(index).zfill(2)
    url = project['url']
    # Use a free screenshot service - no key needed for basic use
    ...
    <a class="card" href="{url}" target="_blank">
      <div class="card-thumb">
        ...
        <div class="thumb-fallback">
          <span>{project.get('category','')}</span>
        </div>
...
.card:hover .card-link {{ letter-spacing: 0.18em; }}
...
.card-thumb.thumb-error .thumb-fallback {{
  display: flex;
}}
